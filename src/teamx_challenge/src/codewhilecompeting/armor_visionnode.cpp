...
Mat kernel = getStructuringElement(MORPH_ELLIPSE, Size(1,1));  // 可根据箭头粗细调整
...
void TestNode::callback_camera(sensor_msgs::msg::Image::SharedPtr msg){
    try{
      ...
      preprocess("red", contours_armor_red, kernel_1,kernel_1);
      // preprocessarrow("red", arrow_contours, kernel_5,kernel_5);
...
//     }
//   morphologyEx(maskafter, maskafter, MORPH_CLOSE, kernel_in);
//   dilate(maskafter, maskafter, kernel_5);
//   findContours(maskafter, contours, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
// }
...
void TestNode::gettemplate(vector<Mat>&template_imgs){
  template_imgs.clear();
  for (int i = 1; i <= 5; i++)
  {
    string package_share_dir = ament_index_cpp::get_package_share_directory("player_pkg");
...
void TestNode::publishmsg(const sensor_msgs::msg::Image::SharedPtr &msg){
  referee_pkg::msg::MultiObject msg_object;
        ...
        msg_object.num_objects = armor_red_list.size();
        vector<string> types = {"armor_red_"};
